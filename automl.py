### 事前準備 ###
#
# 前回、この授業用に作成した「AiPro2023」フォルダに移動して…
#
# > python  -m  venv  .automl     ← 新しい仮想環境「.automl」を生成する
#
# > .automl\Scripts\activate.bat    ← 新しい仮想環境「.automl」を起動する
#
# (.automl) > python -m pip install -r requirements.txt   ← ライブラリを一括インストールする
#
# ※一括インストールには時間がかかりますので(10分～？）休み時間の前などに実行するとよいでしょう
#
...
#
# 一括インストールが完了したら…
# (.automl) > streamlit run automl.py    ← 今回のWebアプリ（automl.py）を起動する
#
#
...
def main():
    ...
    if choice == 'グラフ表示':
        ...
        if 'df' in st.session_state:
            ...
            #サイドバーで列を選択
            col = st.sidebar.selectbox('列を選択してください', df.columns)

            # グラフの表示
            st_display_graph(df, col)
